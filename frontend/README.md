...
- **语言**:TypeScript(`tsconfig.json`,strict;迁移期 `noImplicitAny=false`,新代码请写真类型)。
  `npm run typecheck` 跑 `tsc --noEmit`,已并入 `npm test` 和 CI。
- **模块接线**:模块间引用走真正的 import/export;`Object.assign(window, {...})` 与
  `src/boot.ts` 的 window.React 挂载仅为历史兼容保留,新代码不要再读 window 全局。
  `src/main.tsx` 的 import 顺序:boot → data(demo MC_DATA)→ shared(MCStore)→ 各页面 → live。
- **数据层**:
  - `src/data.ts` — 演示数据(导出 MC_DATA,同时挂 window 兼容),也是后端字段缺失时的兜底形状
  - `src/api.ts` — 后端 HTTP 客户端(带重试/超时,沿用旧前端)
  - `src/live.ts` — 接入层:启动并行预取(自选/持仓/复盘/配置/系统/记忆/覆盖/会话 + 前 30 只股票 K线),
...
- **仍为演示数据的板块**:多空辩论全文、深度研究报告(live 模式下索引里标注「演示 ·」)、
  财务/分析面板、工作流/验证/来源门控展板 —— 后端无落库 API,出现后在 `live.ts` 补映射即可。
