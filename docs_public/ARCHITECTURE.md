...
- **数据从哪来** → **L1 证据 + 数据层**：A/HK/US 行情、财务、新闻/公告与 A 股 QFII 数据全部落本地 SQLite，不上云；Provider Guard 做新鲜度和复权口径护栏，HK/US 只有显式白名单可进入零仓位影子信号。
- **记忆有什么用** → **L0 + L4**：规则、教训、研究索引分层存储；经过 ReviewCase 归因 + 人工确认的结果可从 `pending` 升级为可信记忆。当前决策注入总开关默认关闭，只允许显式查询和影子评估，避免未经证明的历史经验污染下一次判断。
