...
## 附录 B：记忆系统扩面调研（leader，2026-07-05 第二轮）

覆盖 Mem0(47k★,双存储+自动抽取)/Zep-Graphiti(时间知识图谱,arXiv 2501.13956)/Letta-MemGPT(自编辑记忆块)/LangMem/Cognee。全部依赖向量库或图库+embedding 服务 → 维持附录 A 裁决：**不引框架，机制归口**。新增四个机制级增量：

| # | 来源 | 机制 | 明仓转译 | 落点 |
|---|---|---|---|---|
| 6 | Graphiti | **双时间轴**(事实何时为真 vs 系统何时得知)+矛盾时旧事实标记失效而非删除 | 记忆行带 event_time/ingestion_time/invalidated_at 三时间戳 → **as-of 记忆召回**：回放/盲裁/演练场可精确重建"系统当时知道什么",记忆层从此 PIT 干净 | Phase 1 表结构（与附录 A#1 合并实现） |
| 7 | Letta | 自编辑记忆块：小容量"core memory"常驻上下文,大容量 archival 检索 | ContextGovernor 分两层：常驻块(用户风险偏好/纪律规则/当前论点摘要,预算内恒注入)+检索块(FTS 按需) | Phase 1 ContextGovernor |
| 8 | Mem0 | 会话后自动抽取记忆候选(LLM 提取→人审) | profile miner 的实现参照：m63 盘后/周末产出"记忆候选"进治理台,人工确认才转正（§4.1 不自动提升可信度） | Phase 3 miner |
| 9 | 全行业共识 | 记忆按域分仓+权限 | 扩附录 A#5 命名空间至**七仓**：交易纪律/研究论点/个人偏好与约束/操作复盘/外部方法论(如赛道研究员方法增量)/数据源经验(如 iFinD 措辞坑)/系统运维 | Phase 1 namespace 枚举 |

owner 定调(07-05)：记忆系统重要性上调——"不光交易,很多重要信息都该进记忆"；七仓设计即此落点。
验收补充：演练场用 as-of 记忆召回后，"记忆开/关盲裁对照"升级为"记忆开(as-of)/记忆关/记忆全量(穿越)"三臂——第三臂用于量化时间穿越的危害,反证 PIT 召回价值。

### 附录 B 修订（owner 2026-07-05）：治理默认 LLM 初审，人工降为复核

- 记忆候选治理改为 **LLM 默认裁决**（采纳/拒绝/改写,结构化理由）→ 即时生效于提示层，带 `reviewed_by=llm` 标记；人工只做**事后复核/修改/撤销**（周末体检汇总一张"本周新记忆"清单供批量抽查），减轻人工压力。
- 分仓分级例外：**交易纪律、个人偏好与约束**两仓仍需人工确认才生效（错的纪律记忆危害大于收益）；其余五仓 LLM 核准即生效。
- §4.1"不自动提升可信度"据此细化：自动生效 ≠ trusted——LLM 核准的记忆停留在"提示可用"档，trusted 档仍只能人工授予。
- **全局有效性验收（owner 明令,并入整体验收）**：①三臂盲裁（as-of/关/穿越）看裁量质量差异；②纵向指标=注入记忆的被引用率、记忆导致结论改变的案例审计（改对/改错各多少）、token 成本；③"帮倒忙检测"=凡记忆注入后结论翻转的案例进复核清单。不起作用或负作用 → 降档回人工审批制。
