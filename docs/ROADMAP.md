...
| **M68 新闻金字塔生产镜像与风险槽位** | **首批可试用**：真实生产库双跑、事件风险镜像、反事实 API/界面/反馈账本已接线；涨跌方向继续影子验证 | 连续试用并补齐冻结证据重放、持久化提取缓存与 token 对照门 | 事件风险灰度须过数据/触发/可解释性门；方向槽位须另过 M54 IC/ICIR/单调/跨 regime 门并由 owner 确认 |
...
- M68 不改正式 `signals`、production weights、止盈止损、仓位、scheduler 时序和 test2；不自动下单。
- M67 在独立 worktree 推进多市场一致性。M68 不修改多市场 provider、HK/US 数据合同、市场写入边界或 M67 文档段落；与 M67 可能共有的 `ROADMAP/STATUS/CHANGELOG` 只追加 M68 自有段落，合并时按里程碑保留双方内容。
...
- [x] 初始镜像使用有界样本（持仓 + test2 + 标的1 + 稳定对照，默认最多 20 支），保留 legacy 反事实与 `NEWS_SHADOW_ENABLED` 关闭开关。

> 事件镜像已将“L0 高重要性却未触发”直接升为高优先级错误样本，且显示低置信/降级 flags；但仍需前向试用证明所有“低证据剧烈跳变”都能被稳定捕获，因此第二项不提前勾选。

### Phase 4 — 方向线继续影子与独立晋级

- [ ] 方向性指标与风险识别指标分账：事件线看重大事件召回、漏报、归因准确、波动识别和用户帮助度；方向线才看 IC/ICIR/分桶/收益反事实。
