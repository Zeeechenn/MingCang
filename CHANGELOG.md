...
## [v0.8.4] 研究任务与数据保护 / Research tasks and data safeguards（2026-09-24）

### Added / 新增

- 现有个股研究区增加 API、CLI、MCP 共用的零调用任务准备，绑定问题、标的、日期范围、期限、证据版本和输出 token 预算；来源追查显示数值、单位、计算口径及缺失项。
- 人工接受、修改、拒绝保存支持/反对证据、未知项与后续验证，追加观察记录；任务台账在调用前原子预约，刷新只查询状态，已完成请求重放既有结果。
- 增加显式、离线的新闻证据审计及固定池/全市场研究准备入口；历史技术回放接入真实行情、共享新闻输入和因子校验。未完成的模型臂与净值仍标为缺失。

### Changed / 变更

- 本地停止与远端结果未知分别记录；流式读取失败不重复 POST，预约前明确拒绝可重新准备，未知调用状态不自动重试。
- 严格行情刷新在调用提供方前拒绝历史混源/混复权，候选不完整或价格跳变时保留旧行；盘前显式保护模式返回 complete/partial/blocked，默认调度保持原状。
- 统一研究检查与文档权威入口，保留原有整体 UI，仅扩展现有研究逻辑。

### Fixed / 修复

- 常规单股上下文与 Piotroski 长期分析采用严格财务时点和缺失值口径；旧标签显示生成时质量及有效期限，缺失财务不再当成负面因子；中国本地截止时点与 UTC 抓取时间统一比较，来源列表同样排除截止后资料。
- A 股财务优先读取摘要中的现金流绝对金额、净资产及比率，保留字段来源；显式补齐保留已有数值和披露日，晚获取数据不能回流到历史截止时点。
- 深研分开统计来源新鲜度与公司/主题文本相关性，证据不足不再输出模板催化或冒充检索成功；主题报告显示长期研究维度缺口及本次角色执行边界。
- 新增本地离线研究入口；新闻按获取时点可见，长期背景与当天新鲜来源分开。研究队列损坏时拒绝覆空，保存使用原子替换，陈旧待办可显式转为待复核，已完成历史归档保留。
- 行情刷新按明确的目标日期计算窗口；拒绝的候选不删除既有日期，删除与插入失败一并回滚。严格来源与因子预热仍按既有显式启用合同执行。

### Safety / 安全边界

- 生产价格维护、冻结模型试验、官方信号和历史面板不因这些代码修复自动重跑或晋升；历史混源、财务覆盖和时点缺口仍保留阻塞。
- 单次应用请求预约不等于提供方底层仅一次调用；输出 token 预算不等于账单上限。本地停止不认证远端取消。
- 新闻方向、模型优劣和投资收益未获认证；发布不启用经济试验或真实交易。

### Verification / 验证

- 隔离的新锁环境完整 `make verify` 通过：后端 2,594 passed / 14 skipped，前端 58 项、构建、ESLint、桌面/手机 Chromium 冒烟通过；Ruff、mypy 382 文件、文档权威和发布卫生检查通过。版本与锁文件一致，锁定依赖审计未发现已知漏洞。
...
- 模型试验增加冻结登记、授权与候选代码核验，以及按截止时点隔离的逐臂账户请求上下文；默认关闭的 v3 候选将完整授权池的逐臂账户接入不可变请求，并用注入式假提供方验证。失败预约计入总量，未知身份/费用和数据缺口显式保留。
...
- 定向更新锁定的 AnyIO 至 4.14.2、Soup Sieve 至 2.9，修复发布审计发现的四条已知依赖漏洞；不扩大依赖升级范围。
- 保留历史价格漂移阻塞、冻结试验和既有账本；本次发布不执行真实交易、不激活经济试验，不把测试通过或连续运行当成收益证据。

### Verification / 验证

- 隔离 `make verify` 全部通过：新锁独立环境后端 2,493 passed / 14 skipped；之后 v3 模型/架构专项 71 passed（含 14 项新增模型用例），合计 2,507 个不同通过用例。前端 51 项、构建、ESLint、桌面/手机 Chromium 冒烟通过。mypy 379 文件、版本/锁文件、文档权威、发布卫生和 MkDocs strict 通过。
...
- 显式启用的严格研究输入：财务缺失保留 NA、按披露日过滤并约束去年同期比较、
  标签生成/有效期检查、风险优先文本及预算不足拒绝；默认消费者保持兼容。
- 独立持仓/用户约束草稿及只读 copilot 候选预览；未知持仓不套试错仓位，禁买不自动清仓，
  原提议与草稿分开，永远等待复核、不执行或写入 ResearchState。
- 决策台实验规格与回执离线检查，涵盖两臂身份、预算、失败日期及观察原文件完整性；
  只验证声明/证据结构，代理价格降级，永远不认证收益。
- 注入式单次观察记录器保存传入 provider 的请求原文、返回、用量和失败；独立 attempt
  拒绝覆盖及路径越界，以 FakeProvider 验证，不默认调用真实模型或启动实验。
- 离线实验会话控制冻结两臂小型预算/输入计划；调用前持久预留累计次数与费用，失败和
  进程中断不自动退款。加入并发锁、重复attempt保护、记录身份核对、跨臂超限停止及只读汇总。
  仅显式注入provider，未接入真实治疗臂、调度器或生产链；本地hash不是可信事前登记认证。

### Changed / 变更

- 沿用 Roadmap 与开发者指南承接决策台方向和 GPT-6/Claude 治疗臂验证；自动加载的
  AGENTS/CLAUDE 指令保持原文。生产数据、日跑、风险政策、旧交易轨迹未更改。
...
## [v0.8.1] P0 基础证据与价格安全 / P0 evidence foundations and price safety（2026-09-03）

### Added / 新增

- **P0-R 独立现金 NAV 对照证据**：新增只读快照输入的真实现金账本回放和手动 CLI，
  覆盖 next-open、T+1、费用、停牌/锁价、量能分批成交、保守止损、拆股/现金分红、
  仓位上限和反转延迟，并冻结 control/execution/input/replay ID；结果只作独立诊断，
  不接 scheduler、正式信号、仓位、权重或止损。快照缺权威公司行动 ledger 或价格口径
  未清零时明确保留 caveat/`blocked_price_basis`，不能作为收益晋升证据。
...
- **P0-B1 价格重基工具安全收口**：价格重基规划必须显式指定数据库，默认只 dry-run；
  ...
  参数。日常写入路径增加默认关闭的 strict guard，临时测试可阻断 mixed/missing provider、
  adjustment 和重基漂移；当前 One Loop caller 未启用。本批未执行 P0-B2，也未修改任何生产数据库。
...
- **P0-G One Loop 默认起算日统一**：连续性审计、健康面和默认 CLI 路径共享同一
  canonical start-date 来源，operator CLI 对临时改写 fail-closed，输出直接列出 compared/
  complete 日期集合；未改变既有日集合、批次选择、完成判据、scheduler 时序或生产写入。

### Verification / 验证

- 2026-09-03 隔离候选树：聚焦回归 `96 passed`；Ruff、release hygiene（扫描
  774 个文件、51 行 allowlist）、文档 authority、mypy（366 个源文件）全绿；后端
  `2088 passed, 14 skipped, 1 warning`；前端 `37/37`、TypeScript、
