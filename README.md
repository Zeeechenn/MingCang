...
> 三路分析师 → 加权综合 → **一票否决**融合 → 长期标签（值得持有 / 估值偏高 / 观望 / 规避），**默认开启、可逐个开关**。**供应链瓶颈分析方法论（Serenity）**已就绪、observe-only，默认关闭（`long_term_serenity_enabled=False`）、暂无生产入口调用，尚未灰度接入长期标签；QFII 资金流等更多框架持续评估中。
>
...
- **新能力要先用真实结果证明，才能影响决策。** 这也是当前的实际状态：完整的研究闭环（L0–L4）已经落地，但默认休眠、量化层关闭；任何更强的信号或模型，都要先通过前向证据门控（IC / ICIR、独立样本、数据质量），并经你确认，才逐层激活。在那之前，生产信号只用一套朴素公式（技术 0.6 + 情感 0.4 + ATR 移动止损）。
