...
明仓不是量化交易系统，也不是 AI 选股器。它不承诺收益、不替你下单、不替你拍板；它做的是把你关心的股票、赛道、观点、风险和复盘结果组织起来，让 AI 帮你扩大扫描半径、反驳假设、补齐证据。被结果验证过的经验会继续沉淀和回测，但当前默认保持影子模式，不进入 AI 判断或官方动作。
...
- **研究入口**：你用自然语言提问，明仓自己调度本地研究、信号和风险上下文；历史记忆只在你明确查询时返回。
...
| 查看记忆 | “过去我在半导体票上踩过哪些坑？只把历史记录列出来，不要改变当前建议。” |
...
- Python 3.11 或更高版本；主 CI 使用 3.11，并对 3.12 / 3.13 跑兼容性契约。
- Web 工作台开发与构建需要 Node.js 20；只使用 Agent / API 时不需要 Node.js。
- 推荐使用 `uv` 按 `uv.lock` 安装可复现依赖；也可使用 Docker Compose（需 Docker Compose v2）。

用 `python --version`、`node --version` 和 `docker compose version` 可确认本机环境。真实数据库升级由 `init_db()` 的幂等运行时迁移完成，升级前仍建议备份本地 SQLite 文件。
...
```text
帮我看一下 300308，结合信号、新闻和长期标签，给我一个研究结论。
```
...
| 记忆系统 | 继续记录、验证和回测历史经验；默认仅供显式查询和影子评估，不影响 AI 判断、信号、仓位或止损。 |
...
这个案例体现了明仓的核心哲学：**亏一笔不可怕，可怕的是没有可复核的归因。** 复盘结论会进入影子记忆继续验证；在前向评估证明有效前，不自动改变下一次判断。
...
| 工具 | 用途 |
|---|---|
| `mingcang_project_context` | 持仓、自选、记忆健康摘要、配置概况；不注入记忆正文 |
| `mingcang_stock_context` | 单只股票：信号、新闻、标签、研究 copilot 影子结论；记忆正文默认关闭 |
| `mingcang_memory_context` | 用户明确要求时只读查询历史经验，不改变当前建议 |
...
| 🧭 **Serenity 瓶颈框架** | Serenity chokepoint skill / 报告门方法论 | 独立分析器已退役；保留检查清单与可选报告门接口，仅显式传入 Serenity 结果才执行加严检查，不参与长期投票或生产信号 |

这些框架属于长期研究层，给的是赛道与个股的中长期判断，不直接改每日信号。每日信号仍由明确规则、风险线和证据门槛约束。

实际运行取决于入口、有效缓存和功能开关。专题深研的五段角色是规则模板，不能当成上述分析师团或多轮 LLM 辩论已执行；报告会分别显示缓存投票、缺失证据与本次执行状态。可用 `python -m backend.tools.m63_research --offline --target 000858 --output-dir /tmp/mingcang-research` 仅从本地数据生成报告。
...
说人话就是：先记下“为什么值得研究”，再判断“现在能不能交易”，交易后记录“为何持有/何时退出”，结果出来后复盘“这次教会了什么”。被结果验证过、经人工确认的经验可进入可信记忆账本，但当前仍只用于显式查询和影子评估。完整架构见 [docs_public/ARCHITECTURE.md](docs_public/ARCHITECTURE.md)。
...
| [docs_public/ARCHITECTURE.md](docs_public/ARCHITECTURE.md) | 分层架构、Case 类型、融合逻辑完整说明 |
...
| [docs_public/WHY_NOT_AI_STOCK_PICKER.md](docs_public/WHY_NOT_AI_STOCK_PICKER.md) | 为什么明仓不是 AI 选股器：LLM 边界、ATR 纪律、记忆门控 |
| [docs_public/ningde_live_sample.md](docs_public/ningde_live_sample.md) | 宁德时代活样本：从自然语言到证据链、影子结论和风险提示 |
