...
> **研究驱动集中持仓 + 纪律执行 + AI 增强判断**的本地 A 股研究工作台。
> 它不是量化系统：调参调的是风控参数、复盘节奏和证据门槛，不是预测参数；不承诺收益，不替你下单。

**明仓是一个本地优先的个人 A 股研究操作系统**：你负责 alpha 与最终判断，AI 负责广度扫描与证伪，系统负责把判断、纪律和结果沉淀成一套会成长的记忆。
...
> 信号给出分档建议与 ATR 风险线，不预测涨跌、不喊必涨。新闻情感由 LLM 读取当日新闻打分。
...
| 你想做的事 | 明仓怎么接 |
|---|---|
| **研究一只票** | `mingcang stock 000001` 拉出官方信号、新闻、标签和研究 copilot 影子结论，作为只读研究上下文 |
...
> 截图和 UI 需要在浏览器打开 <http://127.0.0.1:5173> 查看；命令行只负责启动服务，不会直接显示前端页面。

> demo 库和真实库是两套数据：demo 库只有贵州茅台 / 中际旭创 / 中国平安三只示例股。装好真实环境后再查这几只票，如果它们不在你自己追踪的股票池里，会查不到信号或长期标签（返回为空），这是正常的——换成你自己关注的股票代码即可。
...
你会拿到：官方信号（试错 / 关注 / 观望 / 规避）、最近新闻与情感、长期标签、研究 copilot 的影子结论，以及它列出的风险和待验证问题。需要更深的调研时，让它跑一轮 deep research：
...
明仓按交易节奏分了六个工作流入口：盘前看、盘中记、盘后决、周末体检、研究 `<目标>`、喂观点；底层由 4 个模块命令承载，其中 `m63_daily` 通过 3 个 mode 覆盖盘前 / 盘中 / 盘后：
...
| 入口 | 命令 | 何时用 | `--no-llm` 影响 |
|---|---|---|---|
| 盘前看 | `python3 -m backend.tools.m63_daily --mode premarket` | 开盘前看隔夜公告、新闻、事件和持仓风险线 | 无需 LLM；规则检查照常 |
| 盘中记 | `python3 -m backend.tools.m63_daily --mode intraday` | 盘中只记录观察哨触发、风险线接近和价格异动 | 无需 LLM；规则检查照常 |
| 盘后决 | `python3 -m backend.tools.m63_daily --mode postmarket --no-llm` | 收盘后串起补数、观察哨、面板、触发路由和人话报告 | 跳过 LLM 裁量、新闻情感 accrual scoring 和自动标签刷新；规则部分照常 |
| 周末体检 | `python3 -m backend.tools.m63_weekly --no-llm` | 一周结束看归因、漏网触发器、陈旧论题和数据健康 | 跳过本周复盘归因 LLM；触发器审计、数据健康、队列升级照常 |
| 研究 `<目标>` | `python3 -m backend.tools.m63_research --target 300308 --no-llm` | 临时研究一只票或一个主题，更新标签、深研、copilot 和观察哨 | 跳过标签、深研、copilot LLM；数据补齐和观察哨写入照常 |
| 喂观点 | `python3 -m backend.tools.m63_opinion --text '<观点内容>' --source manual --no-llm` | 把外部观点或自己的想法归档进观察哨体系 | 只归档不分析；不会做观点差异 LLM 与触发排队 |

Pi 终端里直接说"盘前扫一遍"、"收盘后复盘一下"即可。信号里包含当日建议、ATR 移动止损位、组合暴露和数据质量预警——明仓不替你下单，只给纪律。
盘后面板会同步给出风险对应的 `protective_action`、止损贴身程度和财务 / 数据质量旗标，方便把裁量判断落到可检查规则。
...
它不会因为论题"听起来有道理"就抬高信号分——只有结果兑现、复盘通过，这条判断才会升级成可信记忆，喂给下一次研究。
...
前面几节够日常使用。这一节是给想搞清楚"底下到底怎么跑"的人看的，涉及一些内部术语，第一次看不用死抠，跳过不影响用产品。
...
> 三路分析师 → 加权综合 → **一票否决**融合 → 长期标签（值得持有 / 估值偏高 / 观望 / 规避），**默认开启、可逐个开关**。**供应链瓶颈分析方法论（Serenity）**已就绪、**observe-only**（只做研究观察，不接入任何交易或打分，纯只读），默认关闭（`long_term_serenity_enabled=False`）、暂无生产入口调用，尚未灰度接入长期标签；QFII 资金流等更多框架持续评估中。
>
...
| ⏳ **防未来函数（PIT）** | 回测按 as-of 时点取数（PIT = Point-In-Time，只用当时能看到的数据），杜绝用未来数据"作弊" |
...
| 能力 | 当前状态 |
|---|---|
| M59 盘后面板 | 四条硬规则已落地：风险警示必须给保护动作，观望类结论必须给再评估触发，持仓体检展示 ATR 距离和风险旗标，财务质量旗标进入候选区与体检。LLM 裁量层是 reference-only，灰度默认关，不改官方信号。 |
| M60 观察哨 + 触发器 | 观察清单、盘后三类触发检测、面板跟进候选区和触发后 LLM 确认层已落地；龙虎榜、资金流、价格异动和主题联动会进入待复核队列。第二时间影子台账 observe-only，不改官方信号。 |
| M63 三触点 | 盘前、盘中、盘后串成日常节奏，再加周末体检、随时研究和观点归档；报告层会把术语翻成人话，并用语言守卫屏蔽操作性表述。 |
| 数据基建 | 源手册库、数据合同、降级分级、PIT 类型门和统一上下文包已接入，重点是可追溯、可降级、可复核，而不是盲信单一数据源。 |
| 盲裁验收 | 判断类功能可以用跨模型盲裁工具做 A/B 验收，裁判看不到答案归属，再用真实结局和多数票做归档。 |
| Web 日常页 | 前端已有"日常"页(`/daily`)，用于查看 M63 日常报告、研究队列和 M59 裁量参考卡；截图/UI 仍以浏览器中的本地页面为准。 |

术语补一句：**观察哨**只扫描已关注标的是否出现价格、资金、新闻或主题联动变化，触发后进入盘后复核，不是操作指令；**触发器**把异常变化记录成待复核事项，用于提醒继续研究，不等于结论。
...
四类"案卷"（Case）说人话就是：先记下"为什么值得研究"（ResearchCase），再判断"现在能不能交易"（SignalCase），交易后记"为何持有/何时退"（PositionCase），结果出来后复盘"这次教会了什么"（ReviewCase）——首尾相接成一个闭环。五层（L0 记忆 → L1 证据 → L2 论题 → L3 信号/持仓 → L4 复盘/校准）分别承载这几步，彼此可链接、可审计。→ [完整架构说明](docs/ARCHITECTURE.md)
...
| PAPER_TRADING.md | 纸上交易战绩入口（**本地文件**：纸上交易记录属个人数据不入公开库；公开读者看上方「测试1/测试2」战绩节即可） |
...
明仓的愿景，一句话：**让 AI 放大你的判断，而不是替你拍脑袋。** 你和你信得过的研究者、成熟框架负责判断；明仓负责盯住、查漏、复盘，并把验证过的经验沉淀成记忆。

**研究上坚持的原则：**

- **判断主要来自人和成熟框架，不靠模型猜价格。** 主力是你自己，以及景气、财务质量、供应链等框架给出的判断；靠价格走势"算命"这条路回测过，没有效果。
- **AI 只做两件事：扩广度、挑毛病。** 帮你扫到一个人看不过来的线索（永远是待验证的猜想），并替你反驳假设、盯失效条件、在亏损前预警。
- **新能力要先用真实结果证明，才能影响决策。** 这也是当前的实际状态：完整的研究闭环（L0–L4，见[深入了解](#深入了解)）已经落地，但默认休眠、量化层关闭；任何更强的信号或模型，都要先通过前向证据门控（IC / ICIR——衡量一个信号历史上到底有没有预测力的统计指标、独立样本、数据质量），并经你确认，才逐层激活。在那之前，生产信号只用一套朴素公式（技术 0.6 + 情感 0.4 + ATR 移动止损）。
- **只记住被结果验证过的经验。** 一条判断对不对，要等结果出来、复盘通过才算数——不会因为"讲得有道理"就被记成真理。

**工具上，接下来要做的：**

- **用前向证据逐层激活闭环。** 持续积累纸上交易和前向样本，验证一层、打开一层，而不是一次性上线。
- **接入港股和美股。** 现在 A 股是主战场，港 / 美股还是只读研究态；下一步做成和 A 股一样可研究、可跟踪、可复盘的完整链路。
- **打磨前端后端、做成好用的软件。** 让不写代码的人也能一键装好、开箱即用，而不只是一套给开发者的脚本。
