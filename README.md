...
**v0.2.2（2026-06-03）** 发布 M41 A/HK/US 只读数据门面：三市场七层能力目录、HK/US daily price bridge、显式外部数据探针、probe summary、global-data API/CLI、canonical schema/PIT gate、价格质量闸门和 probe health ledger 已进入公开主线。

HK/US 仍是 read-only research context：不生成 official signals、不进入 postmarket batch、stop-loss check、long-term constraints、position sizing 或 composite_score。M26 / M27 的量化与 Kronos 证据闭环结论不变，生产默认继续 `WEIGHT_QUANT=0.0` / `kronos_enabled=false`。
