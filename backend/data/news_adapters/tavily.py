"""Tavily title-only wrapper for the M54 news adapter layer."""
...
class TavilyAdapter:
    ...
    def fetch(self, symbol: str, window: NewsWindow) -> list[NewsEvidence]:
        """Fetch Tavily titles as title-only evidence.

        Tavily's existing wrapper returns only titles. It has no canonical URL,
        body text, or real publication timestamp, so freshness uses
        ``window.as_of`` when supplied and otherwise fetch time.
        """
        fetched_at = datetime.now(UTC).replace(tzinfo=None)
        published_at = window.as_of or fetched_at
        days = window.lookback_days if window.lookback_days is not None else 1
        ...
        return [
            ...
            for title in fetch_titles_tavily(
                symbol,
