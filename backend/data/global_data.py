...
CANONICAL_SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "capital_flow_storage": {
        "required_fields": ["symbol", "trade_date", "provider", "fetched_at"],
        "optional_fields": ["main_net", "super_large_net", "large_net", "medium_net", "small_net"],
        "pit_date_field": "trade_date",
        "decision_visibility_rule": "stored fund-flow trade_date must be <= decision date",
    },
    "research_reports": {
        "required_fields": ["symbol", "title", "org_name", "publish_date", "provider"],
        "optional_fields": ["rating", "eps_forecast_y1", "eps_forecast_y2", "info_code", "fetched_at"],
        "pit_date_field": "publish_date",
        "decision_visibility_rule": "research report publish_date must be <= decision date",
    },
    "lhb": {
        "required_fields": ["symbol", "trade_date", "provider"],
        "optional_fields": ["reason", "net_buy_amount", "buy_seats_json", "sell_seats_json", "fetched_at"],
        "pit_date_field": "trade_date",
        "decision_visibility_rule": "LHB trade_date must be <= decision date",
    },
    "corporate_events": {
        "required_fields": ["symbol", "event_type", "title", "event_date", "provider"],
        "optional_fields": ["detail", "fetched_at"],
        "pit_date_field": "event_date",
        "decision_visibility_rule": "corporate event_date must be <= decision date",
    },
    "holders": {
        "required_fields": ["symbol", "report_date", "provider"],
        "optional_fields": ["total_shares", "float_shares", "top10_json", "holder_count", "fetched_at"],
        "pit_date_field": "report_date",
        "decision_visibility_rule": "holder report_date must be <= decision date",
    },
    "overseas": {
        "required_fields": ["symbol", "name", "snap_date", "provider"],
        "optional_fields": ["close", "chg_pct_1d", "chg_pct_20d", "note", "fetched_at"],
        "pit_date_field": "snap_date",
        "decision_visibility_rule": "overseas snapshot date must be <= decision date",
    },
    "sector": {
        "required_fields": ["symbol", "name"],
        "pit_date_field": None,
        "decision_visibility_rule": "sector classification row must include symbol and name",
    },
    "news": {
        "required_fields": ["symbol", "title", "published_at", "provider"],
        "optional_fields": ["url", "content", "fetched_at"],
        "pit_date_field": "published_at",
        "decision_visibility_rule": "news published_at must be <= decision date",
    },
    "f10": {
        "required_fields": ["symbol", "provider"],
        "optional_fields": ["content", "fetched_at"],
        "pit_date_field": "fetched_at",
        "decision_visibility_rule": "F10 snapshot timestamp must be preserved",
    },
    "derivatives": {
