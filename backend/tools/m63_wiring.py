...
WIRING_MAP: dict[str, WiringEntry] = {
    ...
    "backend.tools.memory_health": {
        "bucket": "manual_only",
        "reason": "记忆健康审计默认只读;维护写入需显式--apply",
    },
    ...
    "backend.tools.long_term_constraint_impact": {"bucket": "manual_only", "reason": "长期标签影响读数是诊断入口,不由 M63 workflow 直接导入"},
    ...
    "backend.tools.m57_trace_capture": {"bucket": "manual_only", "reason": "M57真实深研trace捕获,默认dry-run,仅人工显式apply"},
    "backend.tools.m57_phase2_eval": {"bucket": "manual_only", "reason": "M57/M65四臂盲评验收器,证据类人工触发,不接日常线"},
    "backend.tools.m65_phase2_run": {"bucket": "manual_only", "reason": "M65真实四臂外部模型裁决,仅owner明确授权后人工触发,不接日常线"},
    ...
    "backend.tools.m63_trigger_backscan": {"bucket": "manual_only", "reason": "触发源历史回放验收,人工触发"},
    ...
    "backend.tools.m60_market_temperature": {"bucket": "manual_only", "reason": "打板层盘后温度快照,leader验源后再接自动盘后"},
