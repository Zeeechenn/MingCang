...
_TOOL_REGISTRY: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "recommended_entrypoint": "python3 -m backend.tools.m57_miner --trace-type research.deep_research.complete --pretty",
        "still_runnable": True,
    },
    {
        "module": "backend.tools.m57_trace_capture",
        "category": "maintenance",
        "purpose": "Capture bounded, deduplicated M57 observability traces from completed deep-research decision runs.",
        "read_write_boundary": "Dry-run by default; --apply writes only evolution_traces and never creates candidates, trusted memory, signals, positions, or scheduler state.",
        "recommended_entrypoint": "python3 -m backend.tools.m57_trace_capture --decision-run-id <id> --pretty",
        "still_runnable": True,
    },
    {
        "module": "backend.tools.m57_phase2_eval",
        "category": "evidence",
        "purpose": "Build and evaluate blinded Base/Memory/Serenity/Both research-quality comparisons for M57/M65.",
        "read_write_boundary": "Zero LLM; audit opens SQLite mode=ro; build/evaluate write only caller-selected artifacts and never touch database, trusted memory, signals, positions, scheduler, or test2.",
        "recommended_entrypoint": "python3 -m backend.tools.m57_phase2_eval audit --pretty",
        "still_runnable": True,
    },
    {
        "module": "backend.tools.m65_phase2_run",
        "category": "evidence",
        "purpose": "Run the manual M65 Base/Memory/Serenity/Both live shadow adjudication with frozen evidence, resumable arm outputs, blind judging, and factorial attribution.",
        "read_write_boundary": "Reads MingCang SQLite via mode=ro and local generated research reports; manual execution calls isolated Codex CLI sessions and writes only caller-selected experiment artifacts (default /private/tmp); never writes database, trusted memory, signals, positions, scheduler, or test2.",
        "recommended_entrypoint": "python3 -m backend.tools.m65_phase2_run run --out-dir /private/tmp/mingcang_m65_phase2_<date>",
        "still_runnable": True,
    },
    {
