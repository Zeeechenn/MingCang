...
参考 .pi/skills/track-analyst/SKILL.md（fallback：~/.claude/skills/track-analyst/SKILL.md）的方法论：
  1. 供应链数据核查
  2. 海外领先指标
  3. 周期 vs 结构性
  4. 炒作过滤
...
# 优先级：项目内 .pi/skills/track-analyst/SKILL.md > 用户全局 ~/.claude/skills/...> _FALLBACK_SYSTEM
# 项目内版本随 git 同步到 GitHub，避免在其他机器/服务器上退化到精简兜底版
_PROJECT_ROOT = Path(__file__).resolve().parents[3]
SKILL_MD_CANDIDATES = (
    _PROJECT_ROOT / ".pi" / "skills" / "track-analyst" / "SKILL.md",
    Path.home() / ".claude" / "skills" / "track-analyst" / "SKILL.md",
)

# 兜底用的精简 system prompt（SKILL.md 不可达时用）
_FALLBACK_SYSTEM = """你是赛道研究员，对 A 股科技/硬件赛道做研究级判断。
方法论五层：
1. 供应链数据核查 — 锁单/排产、原材料涨价、交期、产品单价上涨（要量化数据，不接受叙事）
2. 海外领先指标 — 海外光器件/激光/光纤龙头订单 backlog、大客户战略投资、需求上修
    ...
    for path in SKILL_MD_CANDIDATES:
        try:
            if path.exists():
                ...
                logger.debug("track_analyst SKILL.md loaded from %s", path)
                return raw
...
def analyze(symbol: str, name: str, db) -> LongTermReport:
    """主入口"""
    if not settings.long_term_track_enabled:
        return LongTermReport(
            role="track", score=0, confidence=0,
            label_vote="观望", key_findings=["赛道研究员分析师已禁用"],
        )
    ...
    if moves.get("move_180d") is not None and moves["move_180d"] > 50 \
            and label_vote == "值得持有":
        logger.info("track_analyst %s: 180日涨幅 %.1f%% > 50%%，强制降级值得持有→估值偏高",
                    symbol, moves["move_180d"])
        label_vote = "估值偏高"

    confidence = min(1.0, abs(score) / 60)
    logger.info("track_analyst %s: score=%.0f → %s", symbol, score, label_vote)
