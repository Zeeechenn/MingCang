...
- M29.3 readiness guard（2026-06-01）：`backend.tools.m29_forward_readiness` 已接入，只读判断 test3 universe 是否具备完整新增交易日与 1d/3d/5d future-return 覆盖；只有三条 exit horizon 同时 ready 才输出下一轮命令，否则输出 blockers 且不运行 forward shadow。CLI smoke `/private/tmp/m29_forward_readiness_finish_smoke.{json,md}` 为 `ready_to_run_forward_shadow=false`、commands=[]、`runs_forward_shadow=false`；当前状态为 tooling/provenance-ready、fresh-forward-blocked。
